...
class RecordFinderTest extends FunSuite with Matchers {
  ...
  test("IDs on LHS are also present in RHS of id: listOfIDs:") {
    ...
    /*val lineContainsID = outputRDD
      .filter(line => line(1).isEmpty)
      .map(line => line(1) contains line(0) ) // then verify that ID is contained in listOfIDs
      .reduce(_ && _) // then make sure this is true FOR ALL rows */
